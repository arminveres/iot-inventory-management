...
for line in contents:
    ...
    match = re.search(r"(REVOCATION|REV_RECEIVED): time: (\d+), node: (.+)", line)
    if match:
...
for entry in data:
    ...
    elif entry["node"] == prev_node and entry["action"] == "REV_RECEIVED":
...
plt.savefig(f"graphs/revocation_{no_nodes}_nodes_{int(time.time())}.png")
plt.show()
